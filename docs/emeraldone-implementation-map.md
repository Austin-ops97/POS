...
| Feature | Status | Reuse / add |
| --- | --- | --- |
| 14. Social connection center | Done for vault-gated OAuth | Settings → Integrations → Social media. Connect reads `getPlatformCredential` (vault first, host fallback). If the vault is empty the page points at `/admin/builder` Platform credentials. A platform admin sees the missing keys. Facebook and Instagram share the Meta app and the Meta callback. LinkedIn is separate. Tokens are encrypted. The page stays Not connected until OAuth finishes. Reconnect and disconnect stay on the connected account |
...
| Social | Save `META_APP_ID` and `META_APP_SECRET` for Facebook and Instagram, and `LINKEDIN_CLIENT_ID` and `LINKEDIN_CLIENT_SECRET`, in Builder → Platform credentials. The first save generates `SOCIAL_TOKEN_ENCRYPTION_KEY` (32 bytes, base64) and stores the redirect. Register `https://<app>/api/integrations/meta/callback` under Meta → Facebook Login → Valid OAuth Redirect URIs, and `https://<app>/api/integrations/linkedin/callback` under LinkedIn → Auth → Authorized redirect URLs. Instagram uses the Meta callback. Host fallback still works when the vault value is empty. Optional `BLOB_READ_WRITE_TOKEN` for Instagram image URLs. No social passwords |

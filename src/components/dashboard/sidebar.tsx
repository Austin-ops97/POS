...
export function SidebarNav({ onNavigate, className }: SidebarNavProps) {
  ...
  return (
    <nav className={cn("flex-1 space-y-1 overflow-y-auto p-3 sm:p-4", className)} aria-label="Main">
      {NAV_ITEMS.map((item) => {
        ...
        return (
          <Link
            ...
            onClick={onNavigate}
            className={cn(
              "flex min-h-11 items-center gap-3 rounded-lg px-3 py-2.5 text-[15px] font-medium transition-colors",
              active

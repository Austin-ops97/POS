...
import { getImportBatch, rollbackImportBatch } from "@/lib/import/import-service";
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const result = await rollbackImportBatch({ businessId: ctx.business.id, employeeId: ctx.employee.id, batchId: id });
    return NextResponse.json(result);
  } catch (error) {
    return handleApiError(error, "POST /api/import/batches/[id]/rollback");
  }
}
